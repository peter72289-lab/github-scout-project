...
async function assertPublicResolvedHost(hostname) {
  ...
  if (!records.length) throw new Error('Storefront host could not be resolved.');
  records.forEach((record) => {
    if (record.family === 4 && isPrivateIpv4(record.address)) throw new Error('Storefront host resolves to a private IPv4 address.');
    if (record.family === 6 && isPrivateIpv6(record.address)) throw new Error('Storefront host resolves to a private IPv6 address.');
  });
}
...
async function fetchPublicHtml(url, redirectCount = 0) {
  ...
  try {
    ...
    if ([301, 302, 303, 307, 308].includes(response.status)) {
      if (redirectCount >= MAX_REDIRECTS) throw new Error('Storefront redirected too many times to scan safely.');
      const location = response.headers.get('location');
      if (!location) throw new Error('Storefront returned a redirect without a destination.');
      const redirectedUrl = normalizeUrl(new URL(location, url).toString());
      return fetchPublicHtml(redirectedUrl, redirectCount + 1);
    }
...
async function crawlStorefront(storeUrl) {
  return fetchPublicHtml(normalizeUrl(storeUrl));
}
