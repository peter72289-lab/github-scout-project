...
- `v10/data/opportunities.json`: 11 GitHub repo records generated 2026-05-21 from a private Obsidian note (`source` field points at `peter72289-lab/EXPLORING/.../2026-05-01-manual-scout.md`); `summary.dataMode` claims `live-github-api` but each record carries a failed-refresh `liveError`. Not referenced by any v10 HTML or function.
- `v10/data/ecommerce_opportunities.json`: 10 hand-typed records, `source: "GitHub Scout Ecommerce Edition local seed data"`, `generatedAt` midnight. Not referenced.
- `v10/assets/app.js`, `ecommerce.js`, `dossiers.js`, `dossiers.css`, `ecommerce.css`: v8 dashboard code that consumed the JSON above; `ecommerce.js:36` polls `http://localhost:7842/api/opportunities`, which nothing serves. No v10 page loads them (verified by grep).
- `servers/commerce_scan_api.py` (repo root): stdlib HTTP server on `127.0.0.1:8767` that returns raw HTML for a URL; no detection logic; called only by the root v11-v14 launchpad mockups (`scanApi` at v11:1042, v12:1519, v13:1894, v14:2091), by nothing in v10.
...
| `v10/netlify.toml:2`                                                                                                                             | `publish = "."` will serve `*.md`, `supabase/`, `tests/`, `scripts/`, and the 5.3 MB `deploy-*.zip` once v2 deploys                  |
...
| `v10/deploy-1783114711014-*.zip`                                                                                                                 | stale build artifact committed to git                                                                                                |
